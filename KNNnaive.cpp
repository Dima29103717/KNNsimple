...
distance knn::KNNnaive::get_dist(int k)
{
	...
	distance dist(k);
	std::transform(this->__index.begin(), this->__index.begin() + k, dist.begin(), [&](int i) {return this->__data->dis[i]; });
	return dist;
}
...
vecop::class_label knn::KNNnaive::get_labels(int k)
{
	...
	vecop::class_label lables(k);
	std::transform(this->__index.begin(), this->__index.begin() + k, lables.begin(), [&](int i) {return this->__data->__Y[i]; });
	return lables;
}
